...
const DoughnutChartComponent = ({
  ...
  useEffect(() => {
    ...
    const ctx = document.getElementById(
      `${id}_doughnutChart`
    ) as HTMLCanvasElement; // ID 값 -> 캔버스 요소
  ...
  return <canvas id={`${id}_doughnutChart`} width="400" height="400"></canvas>; // 차트 ID값 사용-> 서로 다른 캔버스 생성
};
...
const RenderChartBlock = ({
  ...
  return (
    <div>
      <h1 style={{ fontWeight: "bold" }}>Doughnut Chart Table</h1>{" "}
      <table id="chartTable">
...
export const doughnutchartBlock = createReactBlockSpec(
  {
    type: "chart",
    propSchema: {
      ...defaultProps,
      name: {
        default: "doughnut chart block",
      },
      tableData: {
...
export const insertDoughnutChartBlock: ReactSlashMenuItem<
  BlockSchemaWithBlock<"chart", typeof doughnutchartBlock.config>
> = {
  name: "Doughnut Chart Block",
  execute: (editor) => {
  ...
  hint: "Insert a doughnut chart block!",
};
