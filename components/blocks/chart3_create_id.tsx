...
const PieChartComponent = ({
  ...
  useEffect(() => {
    ...
    // 차트 생성
    const ctx = document.getElementById(`${id}_pieChart`) as HTMLCanvasElement; // Use ID for canvas element
  ...
  return <canvas id={`${id}_pieChart`} width="400" height="400"></canvas>; // Use ID for canvas element
};
    ...
    render: ({ block, editor }: { block: any; editor: any }) => {
      return (
        <RenderChartBlock
          ...
          onChartCreated={() => {
            // Additional tasks when chart is created
          }}
        />
      );
    },
